...
def doThisbot():
    with open("C:/Users/thebo/CSGTempuratureProbes/sqlinfo.txt") as f:
        sqlList = list(f.read().splitlines())
        sql_user = sqlList[0]
        sql_password = sqlList[1]   
    try:
        ...
        print("\nPrinting each laptop record")
        for row in records:
            message.channel.send("DATE = ", row[0], )
            message.channel.send("DEVICE = ", row[1])
            message.channel.send("TEMPERATURE = ", row[2])
